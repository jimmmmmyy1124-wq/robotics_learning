...
### 4. 多目标点导航需要处理连续 Action 请求与状态衔接

任务4不再由人工在 RViz2 中逐次设置目标点，而是由 Python 节点连续向 Nav2 发送导航目标，因此需要保证前一个目标完成后再发送下一个目标，并正确读取 Action 返回结果。

处理方式：

- 使用 `rclpy` 的 Action Client 与 Nav2 `NavigateToPose` 接口通信；
- 为每个目标点依次构造并发送导航请求；
- 等待当前目标导航结束后再进入下一个目标；
- 根据 Action 返回状态输出成功或失败信息；
- 对目标请求失败等异常情况加入基本处理逻辑。

最终三个目标点均能够按照预设顺序完成导航。
